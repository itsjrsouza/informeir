...
# Palavras-chave que indicam linha de rótulo (não dados)
...
def linha_ignoravel(row):
    """Verifica se a linha deve ser ignorada (cabeçalho descritivo ou vazia)."""
    # Se a linha está completamente vazia
    if all(pd.isna(v) or str(v).strip() == '' for v in row):
        return True

    # Verifica células que indicam cabeçalho descritivo
    primeira_col = None
    if 'ANO_CALENDARIO' in row.index:
        primeira_col = str(row['ANO_CALENDARIO']).strip().lower()
    elif 'EXERCICIO' in row.index:
        primeira_col = str(row['EXERCICIO']).strip().lower()

    if primeira_col and any(p in primeira_col for p in PALAVRAS_ROTULO):
        return True

    # Se o CPF está presente mas não contém dígitos
    if 'CPF_SOCIO' in row.index:
        cpf_val = str(row['CPF_SOCIO']).strip()
        if cpf_val and not re.search(r'\d', cpf_val):
            return True
...
def parse(caminho):
    ...
    # Encontra a linha de cabeçalho (contém 'ANO_CALENDARIO')
    header_row = None
    ...
    # Define as colunas e remove linhas acima do cabeçalho
